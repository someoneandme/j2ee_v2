...
public class DOInfoReader {
	 ...
	 * @return
	 */
	public static Table getTable(Object objectDO) {
		return objectDO.getClass().getAnnotation(Table.class);
	}
	 ...
	 * @return
	 */
	public static List<Field> getColumns(Object objectDO) {
		List<Class<?>> classLink = new ArrayList<Class<?>>();
		Class<?> curClass = objectDO.getClass();
		while (curClass != null) {
	...
	public static void main(String[] args) {
		...
		Table table = getTable(studentDO);
		System.out.println("table name:" + table.value());

		// 遍历所有field，获得列注解
		List<Field> fields = getColumns(studentDO);
		for (Field field : fields) {
